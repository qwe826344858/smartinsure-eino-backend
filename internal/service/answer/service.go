...
func (s *Service) GenerateWithHistory(ctx context.Context, message string, intent string, searchResults []schema.SearchResultItem, history []HistoryMessage) (schema.AnswerResult, error) {
	...
	logx.Printf("运行日志", "runtime log", "answer generate_start intent=%s message_chars=%d results=%d history=%d", intent, len([]rune(strings.TrimSpace(message))), len(searchResults), len(history))

	raw, err := s.model.CallText(ctx, BuildAnswerMessagesWithHistory(message, intent, searchResults, history), 0.3)
	if err != nil {
		logx.Printf("运行日志", "runtime log", "answer generate_failed intent=%s duration_ms=%d err=%v", intent, time.Since(startedAt).Milliseconds(), err)
		return schema.AnswerResult{}, fmt.Errorf("generate answer: %w", err)
	}
	result := ParseAnswer(raw)
	logx.Printf("运行日志", "runtime log", "answer generate_success intent=%s raw_chars=%d summary_chars=%d details=%d duration_ms=%d", intent, len([]rune(raw)), len([]rune(result.Summary)), len(result.Details), time.Since(startedAt).Milliseconds())
	return result, nil
}
...
func (s *Service) GenerateStreamWithHistory(ctx context.Context, message string, intent string, searchResults []schema.SearchResultItem, history []HistoryMessage) (<-chan llm.StreamChunk, error) {
	...
	logx.Printf("运行日志", "runtime log", "answer stream_start intent=%s message_chars=%d results=%d history=%d", intent, len([]rune(strings.TrimSpace(message))), len(searchResults), len(history))
